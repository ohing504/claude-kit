...
- 잔존 비용(residual cost) 규칙을 바꾸려면 `tests/test_residual.py`를 먼저 고친다. 그 파일이 잔존 규칙의 정본이다.
- 구독 한도 표본(quota) 규칙을 바꾸려면 `tests/test_quota.py`를 먼저 고친다. 그 파일이 한도 표본 규칙의 정본이다.
