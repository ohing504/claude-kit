...
대충 던진 요청을 반응형 인터뷰로 파고들어 진짜 의도를 뽑아낸 뒤, 다른 실행(ultracode 견고성 작업·Fable 원샷·loop 시드·일반 에이전트)에 붙여넣을 의도 명세(intent spec)로 만든다. 산출물은 프롬프트 형식이지만, 문구를 손으로 튜닝하는 게 아니라 무엇을 시킬지의 의도·맥락을 인터뷰로 정밀화하는 메타프롬프팅 도구 — 되물을 수 없는 원샷·반복 실행일수록 값이 크다. 짧으면 복붙용 코드블록으로, 길면 `.md` 파일로 저장하고 다음 세션에 붙여넣을 짧은 핸드오프 지시문 한두 줄만 준다(터미널을 프롬프트로 꽉 채우지 않는다). 실행은 사용자가 대상 세션에 붙여넣어 한다.
