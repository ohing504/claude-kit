...
`common.md`를 전제하고 프론트엔드에만 걸리는 것을 담는다. 판정 여섯 개다.
...
**FSD의 `entities`와 `features`와 `widgets` 3분할은 도입하지 않는다.** 그 셋을 쓰면 한 대상(`user`)의 코드가 `entities/user/`와 `features/reset-password/`와 `widgets/user-profile-card/` 세 폴더로 나뉜다. 얻는 것은 재사용 판정 하나이고, 그 판정은 파일을 옮길 때 해도 늦지 않다.
...
세그먼트 이름 다섯은 FSD가 정의한 것을 그대로 쓴다. `ui`(UI 표시에 관한 모든 것), `api`(백엔드 상호작용), `model`(데이터 모델과 비즈니스 로직), `lib`(그 슬라이스의 다른 모듈이 쓰는 코드), `config`(설정과 기능 플래그). **만들어진 것만 두고 비어 있을 것을 미리 만들지 않는다.**

## 2. 세그먼트 이름은 쓰임을 가리킨다

`components/`, `hooks/`, `types/`, `utils/`, `actions/`, `modals/`로 나누지 않는다. 파일이 무엇인지만 말하고 무엇을 위한 것인지는 말하지 않는다.
...
이 이름들이 나쁜 이유를 튜토리얼이 셋으로 적는다. 폴더 안에서 코드를 찾는 데 도움이 안 되어 파일을 하나씩 열어 봐야 한다. 무관한 코드를 가까이 붙여 놓는다. 리팩토링이 닿는 범위가 넓어져 리뷰와 테스트가 어려워진다.
...
**도구가 경로를 고정하면 그 경로를 지킨다.** shadcn은 `components.json`의 `aliases`로 `"ui": "@/components/ui"`와 `"utils": "@/lib/utils"`를 고정하고 CLI가 그 경로에 파일을 넣는다. 이름 규칙보다 도구가 고정한 경로가 앞선다(`common.md` 판정 3과 같다).

## 3. 배럴 파일을 쓰지 않는다

배럴 파일은 `index.ts`가 하위 파일을 재export하는 것이고, FSD가 public API를 구현하는 수단이다. 앱 코드에는 두지 않는다.

**비용 셋.** 모듈 그래프가 부푼다 — 배럴에서 하나를 꺼내도 그 배럴이 재export하는 것을 전부 읽는다. 정의로 이동이 실제 구현이 아니라 `index.ts`에 착지하고 `grep`에도 재export 줄이 먼저 걸린다. 같은 슬라이스 안의 파일이 배럴에서 import하면 순환 참조가 생긴다.
...
자기 앱 코드에서도 같은 크기로 나온다. TkDodo의 Next.js 프로젝트에서 페이지 하나가 11k 모듈을 읽어 시작에 5~10초가 걸렸고, 내부 배럴을 걷어낸 뒤 3.5k로 68% 줄었다(<https://tkdodo.eu/blog/please-stop-using-barrel-files>).

**Next.js의 `optimizePackageImports`는 이 비용을 자기 코드에서 걷어내지 않는다.** node_modules 패키지가 대상이고 슬라이스마다 두는 `index.ts`는 해당하지 않는다.
...
**문서에 적지 말고 린트에 건다.** 규칙을 문서에만 적으면 새 팀원이 안 지켜도 아무 일이 없지만 린트는 커밋을 막는다.

Biome을 쓴다면 이 설정이 최소다. 다섯 규칙 중 기본으로 켜지는 것은 `noPrivateImports` 하나이고 나머지는 적어야 돈다(2026-09-02 확인).
...
`noPrivateImports`는 `correctness` 그룹이고 project 도메인에 속해 저장소 전체를 스캔한다. 스캔 비용을 문다고 공식 문서가 밝힌다(<https://biomejs.dev/linter/domains/>).

ESLint를 쓴다면 `import/no-restricted-paths`의 zones로 같은 것을 막는다. 슬라이스가 늘 때마다 zone을 손으로 추가하는 것이 이 방식의 유지비다. FSD를 규격대로 쓴다면 steiger가 규칙을 자동으로 걸지만 beta라서 규칙 이름과 설정 형식이 바뀔 수 있다.
...
- **`_folder`** — 밑줄 접두는 그 폴더와 하위 전체를 라우팅에서 뺀다. colocation이 이미 되므로 필수는 아니고, 앞으로 추가될 Next.js 파일 규약과 이름이 부딪히는 것을 피하는 용도다.
- **`(folder)`** — 괄호로 감싸면 URL 경로에서 빠진다. 사이트 구획으로 라우트를 묶거나 같은 층에서 레이아웃을 여러 개 둘 때 쓴다.
- **`src/`** — `app`을 `src/app`으로 옮겨 설정 파일과 앱 코드를 가른다. 루트에 `app`이 있으면 `src/app`은 무시된다.

**FSD와 함께 쓰면 이름이 부딪힌다.** `app`과 `pages`가 양쪽에 있다. FSD 공식 가이드의 해법은 FSD 레이어에 접두를 붙이는 것(`_app`, `_pages`)이고, 배치는 Next.js의 `app`을 루트에 두고 `src/`에는 FSD 코드만 남기는 쪽을 권한다.

**App Router에서 배럴을 쓴다면 함정이 하나 더 있다.** 서버 전용 모듈이 슬라이스의 `index.ts`로 나가면 클라이언트 컴포넌트가 그 슬라이스를 import할 때 서버 전용 부수효과가 클라이언트 모듈 그래프로 따라 들어가 빌드가 깨질 수 있다. 판정 3을 지키면 이 함정에 걸리지 않는다.

**라우트 진입점과 슬라이스가 어긋나는 지점이 하나 있다.** Wieruch는 기능을 `pages/projects/` 안에 넣는 것을 반대한다 — 기능 폴더 구조가 프로젝트 안에서 일관되지 않게 되고 나중에 다른 페이지에서 재사용하려면 다시 꺼내야 한다. Next.js가 `_folder`로 colocation을 권하는 것과 어긋나므로, 둘 중 무엇을 따를지 저장소마다 한 번 정해 저장소 문서에 적는다.
