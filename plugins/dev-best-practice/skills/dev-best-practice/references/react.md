...
`@biomejs/biome@2.5.11`로 확인한 결과다(2026-09-03). 다른 기능 폴더를 `@/features/a/service`로 부르면 막히고, 같은 기능 안의 `./use-a`와 공용 폴더의 `@/shared/util`은 통과한다. `noRestrictedImports`의 `patterns`는 v2.2.0부터 쓸 수 있다.
