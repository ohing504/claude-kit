...
- **타이포**: 고대비 페어링. 약한 웨이트(100/200) vs 강한 웨이트(800/900). 크기 점프 3배 이상. 디스플레이는 세리프, 데이터·라벨은 모노.
  - **가독성 하한 — 어떤 텍스트도 13px 미만으로 두지 않는다.** 테이블 헤더(`th`)·캡션·범례·작은 주석·모노 라벨이 가장 자주 위반한다. 본문 15px+, 보조 텍스트 13px+. "작게 = 세련됨"이 아니다 — 다시 읽으려고 만든 문서다.
...
| 프로세스·흐름·관계·시퀀스 | **Mermaid** (CDN) | 자동 레이아웃, 좌표 환각 0 |
...
<style>
  ...
  /* 가독성 하한 — 13px 미만 금지(헤더·캡션·라벨이 가장 자주 위반) */
  table{ font-size:14.5px; } th{ font-size:13px; }
  .cap{ font-size:13px; color:var(--lo); }
  /* 다이어그램은 컨테이너 가운데 정렬 */
    ...
    .then(async ({ default: mermaid }) => {
      mermaid.initialize({
        ...
        themeVariables: { background:'#10131a', primaryColor:'#1b202b', primaryTextColor:'#f3efe6',
          primaryBorderColor:'#8aa8cf', lineColor:'#8aa8cf', fontSize:'14px' }
      });
...
### Mermaid 라벨 주의
노드/메시지 텍스트에 `✕ → — '' "" ( )` 같은 특수문자를 넣지 않는다(파싱 깨짐). 화살표는 문법(`-->`)으로, 강조는 일반 단어로. 작성 후 반드시 `scripts/verify_render.sh`로 검증한다.
