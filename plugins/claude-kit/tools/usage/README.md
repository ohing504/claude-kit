...
`usage session`과 `usage corpus`의 모든 출력은 `measured` 필드(JSON) 또는 첫 줄(`--table`)에
"세션 기록에 남은 것만 잰다"는 경계 문구를 담는다. 이 도구를 호출하는 쪽은 출력만 보므로,
문서에만 적으면 그 경계를 알 수 없다.
...
`--by`를 무엇으로 주든 출력 최상위에 `summary`가 함께 실린다 — 필터가 적용된 코퍼스 전체를
요약한 값이다. `sessions`(메인 스코프 수), `sessions_with_compaction`과 `compaction_ratio`
(압축이 한 번이라도 있었던 세션의 수와 비), `scopes`(서브에이전트를 포함한 스코프 수),
`requests`(전체 요청 수)를 낸다.
...
- **`spread`**: `--group-by`로 고른 키(스코프의 첫 `Skill` 호출, 서브에이전트 종류, 프로젝트)로
  세션들을 묶어, 그 키 안에서 스코프별 요청당 잔존의 분포를 낸다 — `mean`, `min`, `median`,
  `p95`, `max`. "같은 종류의 작업"을 이 도구가 미리 정하지 않으므로, 그 키가 실제로 같은 종류를
  묶었는지와 편차가 큰지의 판단은 리포트를 읽는 쪽이 한다. 행은 그룹 전체의 총 잔존(`residual`)
  내림차순이다 — `--top`으로 잘릴 때 총량이 큰 것이 남아야 고칠 가치가 큰 것이 살아남는다.
...
소진율이 0일 때와, 소진율은 있는데 인덱스에 그 창의 요청이 하나도 없을 때는 내지 않는다. 뒤의
경우는 인덱스가 오래됐거나 다른 기기에서 쓴 것이라 0을 그대로 내면 "안 썼다"로 읽힌다.
