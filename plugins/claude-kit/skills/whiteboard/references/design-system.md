...
## 5. Pipeline / Flow 노드 패턴 — 복사·붙여넣기 템플릿

HTML/CSS 직접 구현 플로우의 3대 실패 패턴을 피한다:
...
```
총 너비 = N_노드 × 노드고정폭 + (N_노드 - 1) × 화살표폭(28px)
컨테이너 여유폭 ≈ 1160px (max-width 1240px - padding 80px)

N=4: 4×160 + 3×28 =  724px → 한 줄 OK
N=5: 5×160 + 4×28 =  912px → 한 줄 OK
N=6: 6×160 + 5×28 = 1100px → 한 줄 OK (빡빡)
...
.fbadge   { display:inline-block; font-size:11px; font-weight:600; padding:2px 9px;
            border-radius:999px; margin-bottom:8px; font-family:monospace; }
/* 배지 색상 — alpha ≥ 0.28 필수 */
.fb-custom{ background:rgba(227,138,107,.32); color:#e38a6b; border:1px solid rgba(227,138,107,.7); }
.fb-seam  { background:rgba(158,214,160,.28); color:#9ed6a0; border:1px solid rgba(158,214,160,.68); }
.fb-reuse { background:rgba(127,180,227,.28); color:#7fb4e3; border:1px solid rgba(127,180,227,.65); }
.fb-out   { background:rgba(245,177,61,.3);   color:#f5b13d; border:1px solid rgba(245,177,61,.72); }
.farr     { display:flex; align-items:center; justify-content:center;
            flex:0 0 28px; color:var(--lo); font-size:18px; flex-shrink:0; }
```

```html
<div class="flow-row">
  <div class="fnode">
    <span class="fbadge fb-custom">custom</span>
    <div class="ft">노드 제목</div>
    <div class="fs">부연 설명</div>
  </div>
  <div class="farr">→</div>
  <div class="fnode">
    <span class="fbadge fb-seam">seam</span>
    <div class="ft">다음 노드</div>
    <div class="fs">설명</div>
  </div>
  <!-- ... -->
</div>
...
외부를 `display:inline-flex; flex-direction:column`으로 감싸면 컨테이너가 내용 너비에 맞게 수축 → ↓ 커넥터가 row 1 마지막 노드와 정확히 정렬됨. `width:100%` 금지.

```html
<!-- 외부: inline-flex column. width:100% 쓰면 ↓ 위치가 어긋남 -->
<div style="display:inline-flex; flex-direction:column; gap:0; min-width:0;">
  <!-- Row 1 (L→R) -->
  <div style="display:flex; align-items:stretch; flex-wrap:nowrap;">
    <div class="fnode">…노드1…</div>
    <div class="farr">→</div>
    <div class="fnode">…노드2…</div>
    <div class="farr">→</div>
    <div class="fnode">…노드3…</div>
    <div class="farr">→</div>
    <div class="fnode">…노드4…</div>
  </div>
  <!-- Turn: row 1 마지막 노드 오른쪽 아래 -->
  <div style="text-align:right; color:var(--lo); font-size:20px; line-height:1.2; padding-right:2px;">↓</div>
  <!-- Row 2 (R→L 정렬 — 마지막 노드가 row 1 끝과 정렬됨) -->
  <div style="display:flex; align-items:stretch; flex-wrap:nowrap; justify-content:flex-end;">
    <div class="fnode">…노드5…</div>
    <div class="farr">→</div>
    <div class="fnode">…노드6…</div>
    <div class="farr">→</div>
    <div class="fnode">…노드7…</div>
  </div>
</div>
```
