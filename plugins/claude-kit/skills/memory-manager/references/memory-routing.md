...
메모리 작동 모델과 저장 형식 참조 문서. 판정 규칙, 점검 항목, 정리 절차는 skill.md 인라인.
...
- 함의: 비용은 *프로젝트당 인덱스 길이*가 좌우(누적 폭발 아님). 낭비의 핵심은 *보편 규칙의 cross-silo 중복*이며, 자율 저장 탓에 이 중복은 구조적으로 쌓인다 — 그래서 본 스킬은 점검 후 사용자 확인을 거쳐 정리하는 사후 정리 도구다.

## 2. 저장 형식 (tier 2 프로젝트 메모리)

정리 단계가 재배치, 통합 시 네이티브 메모리 형식을 보존하기 위한 참조.
