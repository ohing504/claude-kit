...
allowed-tools: Bash(git:*), Bash(gh pr:*), Bash(grep:*), Bash(jq:*), Bash(ls:*), Bash(head:*), Bash(tail:*), Bash(cut:*), Read, Write, Agent
---
...
- git: !`git status --short --branch 2>/dev/null; git log @{u}..HEAD --oneline 2>/dev/null || echo "(upstream 없음)"; git worktree list 2>/dev/null`
...
1. compact 요약의 `Pending Tasks`와 대조한다.
2. 사용자가 요청했거나, 요약에 결론 없이 끝난 논의가 있을 때만 아래 명령으로 `pre-compact.txt`를 만들고, `model: haiku` Agent에게 위 대화 항목을 `[L줄번호]`와 짧은 인용을 붙여 찾게 한다. `pre-compact.txt`가 크면 줄 범위를 나눠 여러 Agent에 맡긴다.
3. 훑지 않았으면 [판정](#3-판정한다)에 "compact 이전은 요약으로만 확인"을 적는다.
...
- 변경 버리기, worktree 삭제, 백그라운드 작업 중단은 사용자가 그 번호를 지정했을 때만 실행한다.

## 3. 판정한다

2단계에서 핸드오프로 처분한 항목이 없으면 `잃을 것 없음`과 확인한 항목별 결과를 한 줄씩 출력한다.

있으면 그 항목으로 scratchpad에 `handoff.md`를 쓰고 경로와 `<경로> 읽고 이어가` 한 줄을 출력한다.
...
## 옮겨 둔 곳     ([2단계](#2-처분한다)에서 만든 이슈, 메모리, 문서 경로)
```
