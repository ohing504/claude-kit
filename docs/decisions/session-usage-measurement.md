...
- **다른 클라이언트의 소진이 섞이는 것은 막지 못하고 출력에 경계 문구로 싣는다** — 이 표본은 이 PC에서 Claude Code CLI의 statusLine이 재실행될 때만 관측된다. 데스크톱 앱과 claude.ai 웹에서 쓴 소진은 같은 계정 한도에 반영되면서도 이 PC의 statusLine에도 세션 기록에도 흔적을 남기지 않아, 발생 여부 자체를 이 도구가 알 수 없다. 병렬 세션은 `parallel_sessions`로 표시할 수 있지만 다른 클라이언트는 표시조차 못 한다. 그래서 `usage quota`의 모든 수치에 그 사실을 문구로 붙여, 읽는 쪽이 상한으로 쓰게 한다.
- **API 정가 표를 도구에 넣지 않고 `--plan-monthly`로 월 구독료를 받는다** — 정가는 이 저장소 밖에서 바뀌는데 코드에 박으면 바뀐 줄 모르고 낡은 값이 계속 나온다. 구독 크레딧으로 실행되므로 정가를 곱한 값은 실제 청구와 자릿수도 맞지 않는다. 대신 실제로 낸 월 요금을 인자로 받아 주 요금(월 요금 ÷ 52/12)을 `projected_full`로 나눈다 — 이 값은 정가가 아니라 실제 지출을 쓴 토큰으로 나눈 것이고, 주간 창에만 쓴다. 5시간 창은 하루에 여러 번 열려 월 요금을 배분할 근거가 없다.
...
- **편차 축(`usage corpus --by spread`)은 작업 종류 라벨을 새로 만들지 않고, 기존 대리키(스코프의 첫 `Skill` 호출, 서브에이전트 종류, 프로젝트)를 그대로 쓴다** — 그 안에서 스코프별 요청당 잔존의 분포(`mean`, `min`, `median`, `p95`, `max`)를 낸다. 어느 키가 실제로 "같은 종류의 작업"을 묶었는지, 편차가 큰지는 도구가 판정하지 않고 리포트를 읽는 쪽이 정한다.
