...
- **수집 실패 보고** — 세 출처에 `#N` 언급이 있는데 닫기 키워드도 `Refs`도 없어 참조 줄이 되지 못하면 그 번호를 Step 3 출력에 보고한다. 자동으로 닫기 키워드를 붙이지는 않는다. 바로 위 항목과 다른 것을 다룬다 — 이미 있는 줄이 언제 발동하는지가 아니라, 줄 자체가 만들어지지 않았다는 사실을 알린다.
